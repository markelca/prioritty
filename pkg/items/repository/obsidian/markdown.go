...
func parseFrontmatter(content []byte) (markdown.Frontmatter, string, error) {
	var fm markdown.Frontmatter
	body, err := markdown.Parse(string(content), &fm)
	return fm, body, err
}

// serializeItem creates markdown content from item data.
func serializeItem(input markdown.ItemInput) ([]byte, error) {
